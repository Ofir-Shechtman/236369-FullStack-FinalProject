...
export default function AddNewPoll() {
  ...
  const divOnChange = (e: { target: { value: any; }; }) => {
        if(e.target.value==="1"){multiple_switch(true);}
        if(e.target.value==="2"){multiple_switch(false);}
...
<Stack spacing={3}>
        ...
        <CloseTimePicker name={"Auto Close Time"} value={"AutoCloseTime"} control={control} multiple_enable={multiple_enable}/>
